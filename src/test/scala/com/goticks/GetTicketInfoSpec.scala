...
trait MockWebServiceCalls extends WebServiceCalls {
  ...
  def getEvent(ticketNr: String, location: Location): Future[TicketInfo] = {
      Future {
        if(ticketNr == "1234") {
          TicketInfo(ticketNr, location, event = Some(DateTimeEvent("Quasimoto", Location(4.324218908d,53.12311144d),
  ...
  def callArtistCalendarService(artist: Artist, nearLocation: Location): Future[DateTimeEvent] = {
    Future {
      DateTimeEvent(artist.name,Location(1d,1d), DateTime.now)
    }
  }
}
