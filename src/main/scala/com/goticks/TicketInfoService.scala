...
trait TicketInfoService extends WebServiceCalls {
  ...
  def getTravelAdvice(info: TicketInfo, event: DateTimeEvent): Future[TicketInfo] = {
  ...
  def getPlannedEventsWithTraverse(event: DateTimeEvent, artists: Seq[Artist]): Future[Seq[DateTimeEvent]] = {
    Future.traverse(artists) { artist=>
      callArtistCalendarService(artist, event.location)
    }
  }

  def getPlannedEvents(event: DateTimeEvent, artists: Seq[Artist]): Future[Seq[DateTimeEvent]] = {
    val events = artists.map(artist=> callArtistCalendarService(artist, event.location))
    Future.sequence(events)
  }

  def getSuggestions(event: DateTimeEvent): Future[Seq[DateTimeEvent]] = {
  ...
  def getSuggestionsWithFlatMapAndMap(event: DateTimeEvent): Future[Seq[DateTimeEvent]] = {
  ...
  def getTravelAdviceUsingForComprehension(info: TicketInfo, event: DateTimeEvent): Future[TicketInfo] = {
...
trait WebServiceCalls {
  ...
  def callSimilarArtistsService(event: DateTimeEvent): Future[Seq[Artist]]

  def callArtistCalendarService(artist: Artist, nearLocation: Location): Future[DateTimeEvent]
}
